...
class ActorCritic(tf.keras.Model):
    ...
    def __init__(self, input_shape: tuple, num_actions: int, num_hidden_units: int):
        ...
        self.actor_mu = tf.keras.layers.Dense(
            num_actions, activation="tanh", kernel_initializer=w_init
        )  # estimated action value # mu: -1 ~ 1
        self.actor_sigma = tf.keras.layers.Dense(
            num_actions, activation="softmax", kernel_initializer=w_init
        )  # estimated action value # sd:  0 ~ 1
